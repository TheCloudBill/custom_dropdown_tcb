...
class TCBDropdownMenuItem<T> extends DropdownMenuItem {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const TCBDropdownMenuItem({
    Key? key,
    this.search,
    this.onTap,
    this.value,
...
/// Use the [CustomDropdownTCB.single] factory if only one item needs to be
/// selected.
/// Use the [CustomDropdownTCB.multiple] factory if user must be able to select
/// multiple items at once.
class CustomDropdownTCB<T> extends StatefulWidget {
  /// [items] with __child__: [Widget] displayed ; __value__: any object with
  /// .toString() used to match search keyword.
  final List<TCBDropdownMenuItem<T>>? items;
  ...
  /// List<TCBDropdownMenuItem> itemsListToClearAndFill, int pageNb)] used to
  ...
  final Future<Tuple2<List<TCBDropdownMenuItem>, int>> Function(
      String? keyword,
      String? orderBy,
      bool? orderAsc,
  ...
  @override
  _CustomDropdownTCBState<T> createState() => _CustomDropdownTCBState();
}

class _CustomDropdownTCBState<T> extends State<CustomDropdownTCB<T>> {
  ...
  @override
  void didUpdateWidget(CustomDropdownTCB oldWidget) {
    super.didUpdateWidget(oldWidget as CustomDropdownTCB<T>);
    if (widget.futureSearchFn != null) {
      updateSelectedValues();
    } else {
      updateSelectedItems();
...
class _DropdownDialogState<T> extends State<DropdownDialog> {
  ...
  Future<Tuple2<List<TCBDropdownMenuItem>, int>>? latestFutureResult;
  ...
  Future<Tuple2<List<TCBDropdownMenuItem>, int>>? _doFutureSearch(
      String? keyword,
      {bool force = false}) {
  ...
  /// non selected TCBDropdownMenuItem.
  Widget displayItem(
    TCBDropdownMenuItem item,
    bool isItemSelected,
  ) {
  ...
  /// Builds the list display from the given list of [TCBDropdownMenuItem] along
  /// with the [bool] indicating whether the item is selected or not and the
  /// [int] as the index in the [selectedItems] list.
  Widget listDisplay(
      List<Tuple3<int, TCBDropdownMenuItem<dynamic>, bool>> itemsToDisplay) {
    return Expanded(
      child: Scrollbar(
        ...
        child: itemsToDisplay.length == 0
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  TCBDropdownMenuItem item = itemsToDisplay[index].item2;
  ...
  Widget listWithPagination() {
    ...
    if (futureSearch) {
      ...
      return (FutureBuilder(
        ...
        builder: (context,
            AsyncSnapshot<Tuple2<List<TCBDropdownMenuItem>, int>> snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.done) {
            Tuple2<List<TCBDropdownMenuItem>, int> data = snapshot.data!;
            ...
            itemsToDisplay = data.item1
                .map<Tuple3<int, TCBDropdownMenuItem<dynamic>, bool>>(
                    (TCBDropdownMenuItem item) {
              return (Tuple3<int, TCBDropdownMenuItem<dynamic>, bool>(
    ...
    itemsToDisplay = pagedShownIndexes
        .map<Tuple3<int, TCBDropdownMenuItem<T>, bool>>((int index) {
      return (Tuple3<int, TCBDropdownMenuItem<T>, bool>(
          index,
          widget.items![index] as TCBDropdownMenuItem<T>,
